-- ============================================================
-- Korenkov School — расширение схемы базы данных
-- Выполнить ЦЕЛИКОМ в Supabase: SQL Editor -> New query -> Run
-- Безопасно запускать повторно (использует IF NOT EXISTS)
-- ============================================================
...
alter table students add column if not exists grade_level text; -- класс, напр. "9 класс"
...
alter table lessons add column if not exists subject text;      -- Алгебра, Геометрия, ТВ...
alter table lessons add column if not exists topic text;        -- Производная функции
...
alter table homework add column if not exists trainer_url text; -- ссылка на тренажёр

-- 6. Материалы: видео-разборы ДЗ и записи занятий (расширяем существующую notes
--    как "конспекты", а видео храним отдельно)
create table if not exists materials (
  ...
  file_url text,          -- для конспектов (PDF)
  created_at timestamptz not null default now()
);
...
-- RLS для новых таблиц (тот же временный подход anon-доступа, что и раньше)
