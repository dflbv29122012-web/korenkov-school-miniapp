...
export default function Schedule() {
  ...
  useEffect(() => {
    ...
    async function load() {
      // Собственные (индивидуальные) занятия + занятия групп, в которых состоит ученик
      ...
      let query = supabase.from('lessons').select('*').order('starts_at')
      if (groupIds.length > 0) {
        query = query.or(`student_id.eq.${student.id},group_id.in.(${groupIds.join(',')})`)
      } else {
