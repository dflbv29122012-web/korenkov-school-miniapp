...
export default function TeacherTopics() {
  ...
  async function viewSubContent(topicId, subId) {
    if (viewingSub?.id === subId) { setViewingSub(null); setContent(null); return }
    setViewingSub({ id: subId, topicId })
    setLoadingContent(true)
    const [{ data: hw }, { data: mt }, { data: ls }] = await Promise.all([
      supabase.from('homework').select('*, students(first_name,last_name)').eq('subtopic_id', subId),
      supabase.from('materials').select('*').eq('subtopic_id', subId),
      supabase.from('lessons').select('*, students(first_name,last_name), groups(name)').eq('subtopic_id', subId),
    ])
    setContent({ homework: hw ?? [], materials: mt ?? [], lessons: ls ?? [] })
    setLoadingContent(false)
  }
  ...
  return (
    <div className="screen">
      ...
      <p className="muted">Папки по номерам ЕГЭ. Внутри — темы занятий с привязанными ДЗ, конспектами, видео и записями.</p>
            ...
            <li key={t.id} className="card">
              ...
              {openId === t.id && (
                <div className="card-details">
                  <button className="btn-secondary block" onClick={() => viewTopicContent(t.id)}>
                    👁 {viewingSub?.id === 'topic-' + t.id ? 'Скрыть' : 'Показать что привязано (без темы)'}
                  </button>
                  {viewingSub?.id === 'topic-' + t.id && (
                    loadingContent ? <p className="muted">Загрузка…</p> : <TopicContent content={content} />
                  )}
                    ...
                    <div key={s.id}>
                      <div className="file-row" style={{ cursor: 'pointer' }} onClick={() => viewSubContent(t.id, s.id)}>
                        {renaming?.id === s.id ? (
                          ...
                          <>
                            <span className="file-name">📄 {s.name} {viewingSub?.id === s.id ? '👁' : ''}</span>
