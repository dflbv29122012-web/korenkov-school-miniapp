...
export default function TeacherStudents() {
  ...
  async function updateName(s, first_name, last_name) {
    await supabase.from('students').update({ first_name, last_name }).eq('id', s.id)
    setStudents(prev => prev.map(x => x.id === s.id ? { ...x, first_name, last_name } : x))
  }
            ...
            <li key={s.id} className="card">
              ...
              {openId === s.id && (
                <div className="card-details">
                  <p className="eyebrow">Имя и фамилия</p>
                  <div className="btn-row">
                    <input
                      className="text-input"
                      defaultValue={s.first_name}
                      onBlur={(e) => updateName(s, e.target.value, s.last_name)}
                      placeholder="Имя"
                    />
                    <input
                      className="text-input"
                      defaultValue={s.last_name || ''}
                      onBlur={(e) => updateName(s, s.first_name, e.target.value)}
                      placeholder="Фамилия"
                    />
                  </div>
