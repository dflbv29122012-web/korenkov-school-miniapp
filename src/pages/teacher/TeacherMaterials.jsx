        ...
        <ul className="card-list">
          ...
          {items.map(m => (
            <li key={m.id} className="card">
              ...
              {editId !== m.id && m.video_url && <a href={m.video_url} target="_blank" rel="noreferrer" className="link">▶ Видео по ссылке</a>}
              {editId === m.id ? (
                <>
                  {m.type !== 'note' && (
                    <>
                      <p className="eyebrow">Ссылка на видео</p>
                      <input className="text-input" placeholder="Ссылка на видео (YouTube и т.д.)"
                ...
                <>
                  ...
                  {(m.attachments?.length > 0 || m.video_url) &&
                    <button className="btn-secondary block" onClick={() => { setEditId(m.id); setEditAttach(m.attachments); setEditNewFiles([]); setEditVideoUrl(m.video_url || '') }}>✏️ Изменить</button>}
                </>
              )}
            </li>
          ))}
