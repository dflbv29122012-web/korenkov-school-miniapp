...
export default function Topics() {
  ...
  useEffect(() => {
    ...
    supabase.from('homework').select('topic_id, correct_tasks, total_tasks, status')
      .eq('student_id', student.id).eq('status', 'checked').not('topic_id', 'is', null)
      .then(({ data }) => {
        const map = {}
        ;(data ?? []).forEach(h => {
          if (!map[h.topic_id]) map[h.topic_id] = { correct: 0, total: 0 }
          map[h.topic_id].correct += h.correct_tasks || 0
          map[h.topic_id].total += h.total_tasks || 0
        })
        const pct = {}
        Object.entries(map).forEach(([tid, v]) => { pct[tid] = v.total > 0 ? Math.round((v.correct / v.total) * 100) : null })
        ...
        {topics.map(t => {
          ...
          return (
            <li key={t.id} className="card">
              <div style={{ cursor: 'pointer' }} onClick={() => openTopicFolder(t)}>
