...
export default function TeacherHomework() {
  ...
  async function loadAll() {
    ...
    const [{ data: hw }, { data: st }, { data: gr }, { data: ln }] = await Promise.all([
      supabase.from('homework').select('*, students(first_name,last_name,grade_level)').order('created_at', { ascending: false }).limit(300),
  ...
  async function saveAttachments(hw) {
    await supabase.from('homework').update({ attachments: editAttach }).eq('id', hw.id)
    setEditAttach(null); loadAll()
  }
                  ...
                  <div className="card-details">
                    ...
                    {editAttach ? (
                      <>
                        ...
                        <div className="btn-row">
                          <button className="btn-primary" onClick={() => saveAttachments(hw)}>Сохранить порядок</button>
                          <button className="btn-secondary" onClick={() => setEditAttach(null)}>Отмена</button>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        {hw.attachments?.length > 0 &&
                          <button className="btn-secondary block" onClick={() => setEditAttach(hw.attachments)}>✏️ Изменить порядок / удалить</button>}
                      </>
                    )}
