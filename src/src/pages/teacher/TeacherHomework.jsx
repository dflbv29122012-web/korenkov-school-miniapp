...
export default function TeacherHomework() {
  ...
  async function addHomework() {
    ...
    try {
      if (file) {
        attachment_url = await uploadFile('attachments', file)
      ...
      {!showForm ? (
        ...
        <div className="card">
          ...
          <label className="file-drop">
            📎 {file ? file.name : 'Прикрепить фото или файл ДЗ'}
            <input type="file" accept="image/*,.pdf,.doc,.docx" onChange={e => setFile(e.target.files?.[0] ?? null)} hidden />
          </label>
          ...
          <div className="btn-row">
            ...
            <button className="btn-secondary" onClick={() => { setShowForm(false); setFile(null) }}>Отмена</button>
          </div>
        </div>
      )}
          ...
          {items.map(hw => (
            <li key={hw.id} className="card row-between">
              <div>
                ...
                {hw.attachment_url && (
                  <a href={hw.attachment_url} target="_blank" rel="noreferrer" className="link">
                    {hw.attachment_type === 'image' ? '🖼️ Фото' : '📎 Файл'}
                  </a>
                )}
              </div>
              {hw.status === 'submitted' ? (
